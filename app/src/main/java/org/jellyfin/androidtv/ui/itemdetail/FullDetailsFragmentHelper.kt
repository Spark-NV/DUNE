...
fun FullDetailsFragment.playTrailers() {
    ...
    if (intent != null) {
        ...
        // No remote trailer, open YouTube search in browser
        val title = mBaseItem.name ?: ""
        val year = mBaseItem.productionYear?.toString() ?: ""
        val query = listOf(title, year, "trailer").filter { it.isNotBlank() }.joinToString(" ")
        val browserIntent = android.content.Intent(android.content.Intent.ACTION_VIEW, android.net.Uri.parse("https://www.youtube.com/results?search_query=" + android.net.Uri.encode(query)))
        startActivity(browserIntent)
    }
} catch (exception: ActivityNotFoundException) {
    Timber.w(exception, "Unable to open external trailer or YouTube")
    Toast.makeText(
        requireContext(),
